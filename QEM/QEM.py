    ...
    def generate_valid_pairs(self):
        ...
        for i in range(0, self.number_of_points):
            ...
            current_valid_pairs = np.concatenate(
                [current_point_location * np.ones((valid_pairs_location.shape[0], 1)), valid_pairs_location], axis=1)
            if i == 0:
        ...
        unique_valid_pairs_trans, unique_valid_pairs_loc = np.unique(
            self.valid_pairs[:, 0] * (10 ** 10) + self.valid_pairs[:, 1], return_index=True)
    ...
    def iteratively_remove_least_cost_valid_pairs(self):
        ...
        while (self.number_of_points - self.new_point_count) >= self.ratio * (self.number_of_points):
            ...
            if self.new_point_count % 100 == 0:
                print('Simplification: ' + str(
                    100 * (self.number_of_points - self.new_point_count) / (self.number_of_points)) + '%')
                print('Remaining: ' + str(self.number_of_points - self.new_point_count) + ' points')
                print('\n')

            self.new_point_count = self.new_point_count + 1

        print('Simplification: ' + str(100 * (self.number_of_points - self.new_point_count) / (
